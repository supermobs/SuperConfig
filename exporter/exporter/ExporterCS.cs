...
// 写cs
            foreach (var data in datas.Values)
            {
                ThreadPool.QueueUserWorkItem(ooo =>
                {
                    ...
                    sb.Append("\tstring filename=\"" + jsonfilename + "\";\r\n");
                    sb.Append(
                        "\tList<string> dirs = new List<string>(Directory.GetDirectories(PATH_ASSETS_FOLDER));\r\n");
                    ...
                    {
                        sb.Append("\tpublic " + typeconvert[data.types[i]] + " " +
                        ...
                        foreach (var item in data.funcDatas)
                        {
                            ...
                            for (int i = 0; i < list.Count; ++i)
                            {
                                ...
                                sb.Append(string.Format("\t\t\t_data_{0}[{1}] = delegate({2}) {{\r\n", value.funcName,
                                    id, value.funcTypeParam));
                                Console.WriteLine(list[i]);
                                var trans_list = list[i].Replace("\n", "").Split(new char[] {';'},
                                    StringSplitOptions.RemoveEmptyEntries); // aaa=bbb;ccc=ddd+eee;
                                foreach (var line in trans_list)
                                {
                                    Console.WriteLine(line);
                                    var eqarr = line.Split('=');
                                    sb.Append(string.Format("\t\t\t\t// {0}\r\n", line));
                                    sb.Append(string.Format("\t\t\t\t{0}={1};\r\n", env.TranslateLeft(eqarr[0]),
                                        env.TranslateRight(eqarr[1])));
                                }
                    ...
                    foreach (var g in data.groups)
                    {
                        ...
                        foreach (var t in g.Value)
                            sb.Append(mapTypeConvert[typeconvert[data.types[data.keys.IndexOf(t)]]] + " " +
                    ...
                    for (int i = 0; i < data.keys.Count; i++)
                    {
                        if (data.types[i] == "string" || data.types[i].StartsWith("[]"))
                            continue;
                        sb.Append("\tpublic float data_" + data.name + "_vlookup_" + (data.cols[i] + 1) +
                                  "(int id) {\r\n");
                        // ! id.ToString()
                        sb.Append("\treturn (float)(Get(id)." + data.keys[i].Substring(0, 1).ToUpper() +
                                  data.keys[i].Substring(1) + ");\r\n");
                        sb.Append("}\r\n");
                    }
